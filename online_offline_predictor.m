...
%% Online Training Set and Testing Set
...
start_row = round(7200*(1-x));
trainingSet(start_row:7200,:) = 99;
...
ratingAverage(isnan(ratingAverage)) = 0;
...
filename = sprintf('online_%s_X_%d.mat','NMAE',x);
save(filename,'NMAE')
...
Y_offline = fliplr(Y_CFSVD);
Y_online = fliplr(Y_online);

x = 0.1:0.1:0.8;
plot(x,Y_online,'-o',x,Y_offline,'-o')
ylabel('Normalized MAE')
xlabel('x train test ratio')
title('Plot Online vs Offline prediction (k is fixed at 20)')
